...
```{r ans11}
losslCompanies<-finStatement$公司名稱[finStatement$`營業利益（損失）`<0]
losslCompanies
# loss1Companies
```
...
```{r ans12}
loss2Companies<-finStatement$公司名稱[finStatement$`基本每股盈餘（元）`<0]
loss2Companies
# loss2Companies
```
...
```{r ans13}
a<-finStatement$`基本每股盈餘（元）`<0 & finStatement$`營業利益（損失）`>0
b<-finStatement$`營業利益（損失）`<0 & finStatement$`基本每股盈餘（元）`>0
only1LossCompanies<-finStatement$公司名稱[a|b]
only1LossCompanies
# only1LossCompanies
```
...
```{r ans21}
numExihibitions<-length(exhibitionList)
numExihibitions
# numExhibitions
```
...
```{r ans22}
df_oneShow<- data.frame(title=exhibitionList[[1]][["title"]],
                        time=exhibitionList[[1]][["showInfo"]][[1]][["time"]],
                        location=exhibitionList[[1]][["showInfo"]][[1]][["location"]],
                        locationName=exhibitionList[[1]][["showInfo"]][[1]][["location"]],
                        price=exhibitionList[[1]][["showInfo"]][[1]][["location"]],
                        endTime=exhibitionList[[1]][["showInfo"]][[1]][["location"]],
                        sourcewebName=exhibitionList[[1]][["sourceWebName"]],stringsAsFactors=F)
df_oneShow
# df_oneShow
```
...
```{r ans23}
dataframe_show_info <- function(exhibitionInput){df_oneShow<- data.frame(title=exhibitionInput[["title"]],
                        time=exhibitionInput[["showInfo"]][[1]][["time"]],
                        location=exhibitionInput[["showInfo"]][[1]][["location"]],
                        locationName=exhibitionInput[["showInfo"]][[1]][["location"]],
                        price=exhibitionInput[["showInfo"]][[1]][["location"]],
                        endTime=exhibitionInput[["showInfo"]][[1]][["location"]],
                        sourcewebName=exhibitionInput[["sourceWebName"]],stringsAsFactors=F)
                        return(df_oneShow)}
dataframe_show_info
#  dataframe_show_info
```
